/** @author masterzee001 */
/**
 * Google Cloud Speech-to-Text V2 / Chirp 3, normalized into Videofy's live STT
 * contract.
 *
 * This is the REPO-SIDE live path only. It proves the runtime can construct a
 * Google STT stream for Hausa and Yoruba with ADC-backed authentication and
 * explicit non-secret resource/model configuration. It does not benchmark
...
type GoogleStreamingRecognitionConfig = protos.google.cloud.speech.v2.IStreamingRecognitionConfig;
...
export const GOOGLE_STT_SUPPORTED_LOCALES = ['ha-NG', 'yo-NG'] as const;
export type GoogleSttSupportedLocale = (typeof GOOGLE_STT_SUPPORTED_LOCALES)[number];

export const GOOGLE_STT_LIVE_MODEL = 'chirp_3' as const;
...
export interface GoogleCloudSttConfig {
  ...
  /** Google Speech V2 location selected for qualification/runtime use. */
  readonly location: string;

  /** Recognizer id. `_` is the V2 implicit recognizer, when explicitly selected. */
  readonly recognizer: string;

  /** Live model id. The qualified candidate is currently `chirp_3`. */
...
export class GoogleCloudSttStreamingProvider implements StreamingTranscriptionProvider {
  ...
  constructor(private readonly config: GoogleCloudSttConfig) {
    ...
    if (config.model !== GOOGLE_STT_LIVE_MODEL) {
      throw new Error(
        `Google live STT requires GOOGLE_STT_MODEL=${GOOGLE_STT_LIVE_MODEL}; ` +
  ...
  async openStream(options: StreamingTranscriptionOptions): Promise<StreamingTranscriptionSession> {
    ...
    const session = new GoogleCloudSttSession(
      stream,
      options,
      recognizer,
      this.name,
...
export function normalizeGoogleSttLocale(
  sourceLanguage: string | undefined,
): GoogleSttSupportedLocale {
  ...
  switch (base) {
    ...
    default:
      throw new Error(
        `google-stt live streaming is configured for ha-NG and yo-NG only; ` +
          `received "${sourceLanguage ?? 'unset'}". ` +
          `Use Deepgram or another approved recognizer for other languages.`,
      );
  }
}
...
export async function createGoogleSpeechStreamingClient(
  ...
  return {
    openStreamingRecognize: () => client._streamingRecognize() as unknown as GoogleSpeechStream,
...
class GoogleCloudSttSession implements StreamingTranscriptionSession {
  ...
  async pushAudio(frame: StreamingTranscriptionFrame): Promise<void> {
    if (this.closed) {
      throw new Error('pushAudio after close');
    }
...
function durationSeconds(value: unknown): number {
  ...
  if (value && typeof value === 'object') {
    ...
    if (typeof candidate.toString === 'function') {
      return Number.parseInt(candidate.toString(), 10);
    }
  }
