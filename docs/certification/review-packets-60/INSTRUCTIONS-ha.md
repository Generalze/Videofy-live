# Hausa translation review — 60 items
...
wrong three times on Hausa-adjacent judgements, and knowing what a machine
...
nobody on the team reads Hausa. Your answers decide which engine we are allowed
to use for this language — and if none is good enough, we will use none.

## How to review

Open `review-ha.csv` in Excel, Google Sheets or LibreOffice. Each row has:

- **source_text** — what was written
- **translation_to_review** — what the machine produced
- **direction** — `en->ha` means English was translated INTO Hausa;
  `ha->en` means Hausa was translated into English
