# Yoruba translation review — 136 items
...
wrong three times on Yoruba-adjacent judgements, and knowing what a machine
...
nobody on the team reads Yoruba. Your answers decide which engine we are allowed
to use for this language — and if none is good enough, we will use none.

## How to review

Open `review-yo.csv` in Excel, Google Sheets or LibreOffice. Each row has:

- **source_text** — what was written
- **translation_to_review** — what the machine produced
- **direction** — `en->yo` means English was translated INTO Yoruba;
  `yo->en` means Yoruba was translated into English
