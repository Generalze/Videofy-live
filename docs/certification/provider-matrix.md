...
| OPUS-MT | opus-mt-ha-en | MT | **ha→en** | messaging | 8/8 automated; median 4820 ms. **3 of 8 materially wrong in meaning**: *"Sai gobe da safe."* (see you tomorrow morning) → *"The next morning."* OPUS fallback only behind an approved Google Nigerian route | **DEGRADED · FALLBACK ONLY** · call-live **REFUSED** |
...
| OPUS-MT | opus-mt-ig-en | MT | **ig→en** | messaging | 8/8 automated; median 5658 ms. **4 of 8 materially wrong**, including *"Enwetala m ego ahụ, daalụ."* (I have received the money, thank you) → **"I had found the money, and I lost it."** OPUS fallback only behind an approved Google Nigerian route | **DEGRADED · FALLBACK ONLY** · call-live **REFUSED** |
...
| OPUS-MT | opus-mt-yo-en | MT | **yo→en** | messaging | 8/8 automated; median 6393 ms. **4 of 8 materially wrong**: *"Mo ti gba owó náà, ẹ ṣé."* → *"I have taken the money, you."* OPUS fallback only behind an approved Google Nigerian route | **DEGRADED · FALLBACK ONLY** · call-live **REFUSED** |
...
| **pt->en** | Weights staged and complete, benchmarked directly, but absent from `DEFAULT_OPUS_MT_LANGUAGE_MODELS`, so `findModel` rejects it with `unsupported-language` (400) before any model loads. **"The model works" is established; "the service can serve this pair" is not.** |
| **ha->en, ig->en, yo->en** | Now present in `DEFAULT_OPUS_MT_LANGUAGE_MODELS` only so OPUS-MT can serve as the fallback behind an explicitly approved Google Nigerian route. This does not approve the routes: the seed still has `productionApproved: false`, outstanding human review, unknown commercial-use obligations and `call-live: refused`. |
...
| **Google Translate v3** | Official `@google-cloud/translate` v3 client integrated behind `NIGERIAN_TRANSLATION_PRIMARY=google-cloud` for `en<->yo|ig|ha` only, with OPUS-MT fallback and route-provider enforcement. Still not certified for Nigerian MT: run `scripts/nigerian-translation-screen.mjs`, compare p95 against OPUS-MT, then promote only through the route document after human review/licence/scope approval. |
